...
internal object Utils {
    ...
    fun joinFields(list: Array<String>?): String? {
        return list?.let { TextUtils.join("\u001f", it) }
    }
    ...
    fun joinTags(tags: Set<String>?): String {
        ...
        val processedTags = tags.map { it.replace(" ", "_") }
        return TextUtils.join(" ", processedTags)
    }

    fun splitTags(tags: String?): Array<String>? {
        return tags?.trim()?.split("\\s+".toRegex())?.toTypedArray()
    }
    ...
    fun fieldChecksum(data: String): Long {
        ...
        return try {
            ...
            val digest = md.digest(strippedData.toByteArray(Charsets.UTF_8))
    ...
    private fun stripHTML(s: String): String {
        var result = s
        var htmlMatcher = stylePattern.matcher(result)
        result = htmlMatcher.replaceAll("")
        htmlMatcher = scriptPattern.matcher(result)
        result = htmlMatcher.replaceAll("")
        htmlMatcher = tagPattern.matcher(result)
        result = htmlMatcher.replaceAll("")
        return entsToTxt(result)
    }
    ...
    private fun entsToTxt(html: String): String {
        ...
        val htmlEntities = htmlEntitiesPattern.matcher(result)
        val sb = StringBuffer()
        while (htmlEntities.find()) {
            // Html.fromHtml(String) is deprecated but its replacement isn't available till API24
