 ...
 */
class WanaKanaJava(useObsoleteKana: Boolean) {

    private val mOptions: MutableMap<String, Boolean> = HashMap()
    private val mRtoJ: MutableMap<String, String> = HashMap()
    private val mJtoR: MutableMap<String, String> = HashMap()
    ...
    private fun _allTrue(checkStr: String, func: Command): Boolean {
        for (_i in checkStr.indices) {
            if (!func.run(checkStr[_i].toString())) {
                return false
            }
        }
        return true
    ...
    private fun _isCharConsonant(chr: Char, includeY: Boolean): Boolean {
        val regexp = if (includeY) Pattern.compile("[bcdfghjklmnpqrstvwxyz]") else Pattern.compile("[bcdfghjklmnpqrstvwxz]")
        val matcher = regexp.matcher(chr.toString())
        return matcher.find()
    }
    ...
    private fun _katakanaToHiragana(kata: String): String {
        ...
        for (_i in kata.indices) {
            val kataChar = kata[_i]

            hira += if (_isCharKatakana(kataChar)) {
                val code = kataChar.code + HIRAGANA_START - KATAKANA_START
    ...
    private fun _hiraganaToKatakana(hira: String): String {
        ...
        for (_i in hira.indices) {
            val hiraChar = hira[_i]

            kata += if (_isCharHiragana(hiraChar)) {
                val code = hiraChar.code + KATAKANA_START - HIRAGANA_START
                String(Character.toChars(code))
            } else {
                hiraChar
            }
    ...
    fun _hiraganaToRomaji(hira: String): String {
        ...
        while (cursor < len) {
            chunkSize = minOf(maxChunk, len - cursor)
            while (chunkSize > 0) {
    ...
    private fun _romajiToKana(roma: String, ignoreCase: Boolean?): String {
        ...
        val maxChunk = 3
        var kana = ""
        var kanaChar: String? = null

        val ignoreCaseFinal = ignoreCase ?: false

        while (position < len) {
            chunkSize = minOf(maxChunk, len - position)

            while (chunkSize > 0) {
                chunk = roma.substring(position, position + chunkSize)
                chunkLC = chunk.lowercase()
                ...
                if (chunkLC[0].toString() == "n") {
                    // Convert n' to ん
                    if (mOptions[OPTION_IME_MODE] == true && chunk.length == 2 && chunkLC[1].toString() == "'") {
                        chunkSize = 2
                        chunk = "nn"
                        chunkLC = chunk.lowercase()
                    }
                    ...
                    if (chunk.length > 2 && _isCharConsonant(chunkLC[1], false) && _isCharVowel(chunkLC[2], true)) {
                        chunkSize = 1
                ...
                if (chunk.length > 1 && chunkLC[0].toString() != "n" && _isCharConsonant(chunkLC[0], true) && chunk[0] == chunk[1]) {
                    chunkSize = 1
                    // Return a small katakana ツ when typing in uppercase
                    if (_isCharInRange(chunk[0], UPPERCASE_START, UPPERCASE_END)) {
                        chunkLC = "ッ"
                        chunk = chunkLC
                    } else {
                        chunkLC = "っ"
            ...
            if (mOptions[OPTION_USE_OBSOLETE_KANA] == true) {
                if (chunkLC == "wi") {
                    kanaChar = "ゐ"
                }
                if (chunkLC == "we") {
            ...
            if (roma.length > (position + 1) && mOptions[OPTION_IME_MODE] == true && chunkLC[0].toString() == "n") {
                if ((roma[position + 1].toString().lowercase() == "y" && position == (len - 2)) || position == (len - 1)) {
    ...
    fun isHiragana(input: String): Boolean {
        return _allTrue(input, Command { str -> _isCharHiragana(str[0]) })
    }

    fun isKatakana(input: String): Boolean {
        return _allTrue(input, Command { str -> _isCharKatakana(str[0]) })
    }

    fun isKana(input: String): Boolean {
        return _allTrue(input, Command { str -> isHiragana(str) || isKatakana(str) })
    }
