...
public class AIDictionaryConfigEditorActivity extends AppCompatActivity {
    ...
    private void loadLanguageOptions() {
        ArrayAdapter<String> languageAdapter = new ArrayAdapter<>(this, android.R.layout.simple_spinner_item, LANGUAGE_NAMES);
    ...
    private void handleIntent() {
        long configId = getIntent().getLongExtra("dictionary_config_id", -1);
        if (configId != -1) {
    ...
    private void saveConfig() {
        ...
        if (TextUtils.isEmpty(dictionaryName)) {
            editTextDictionaryName.setError("Dictionary name is required");
            return;
        }
        ...
        AIConfigRepository.saveAIDictionaryConfig(currentConfig);
